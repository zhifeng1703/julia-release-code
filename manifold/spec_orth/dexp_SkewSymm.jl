...
"""
@inline dexp_sxdx(x) = if (abs(x) < 1e-15)  return 1.0 else return sin(x)/x end
"""
    dexp_cxdx(x) -> (cos(x) - 1)/x

This function is well defined everywhere.
"""
@inline dexp_cxdx(x) = if (abs(x) < 1e-15)  return 0.0 else return (cos(x) - 1.0)/x end
...
function test_dexp_para(n = 10)
    ...
    for r_ind = 1:blk_dim
        ...
        MatM1[2 * r_ind - 1, 2 * r_ind - 1] = 0.;
        MatM1[2 * r_ind, 2 * r_ind] = 0.;
