...
function plot_count_curve(hist_exact::Dict{Int,Int}, hist_tol::Dict{Int,Int})
    x1, _ = dict_to_vector(hist_exact)
    x2, _ = dict_to_vector(hist_tol)
    xmax = max(isempty(x1) ? 0 : maximum(x1), isempty(x2) ? 0 : maximum(x2))
    x = collect(1:xmax)
    ye = [get(hist_exact, k, 0) for k in x]
    yt = [get(hist_tol, k, 0) for k in x]
...
function result_complexity(info::PathInfo)
    total = 0.0
    for (m, n) in info.mincut_sizes
        total += complexity_weight(m, n)
    end
    return total
end
...
function print_summary_table(out)
    ...
    println("\nTop test cases by saved complexity:")
    println(" i   j     exact        tolerant        saved       ratio     abs error")
    for x in out.top_saved_cases
        @printf("%2d  %2d  %11.3e  %11.3e  %11.3e  %7.2f%%  %.3e\n",
            x.i, x.j, x.exact_complexity, x.tol_complexity,
            x.saved, 100*x.save_ratio, x.abs_error)
    end
end
...
function test_geodesic_matrix(
    ...
    D_exact = Matrix{Float64}(undef, m, n)
    D_tol = Matrix{Float64}(undef, m, n)
    results_exact = Matrix{PathInfo}(undef, m, n)
    results_tol = Matrix{PathInfo}(undef, m, n)
    geo_init = Matrix{Any}(undef, m, n)

    @inbounds for i in 1:m, j in 1:n
        geo_init[i, j] = geodesic_initial(setA[i], setB[j])
    end
    ...
    for r in 1:nrepeat
        t_exact_rec[r] = @elapsed begin
            @inbounds for i in 1:m, j in 1:n
                path, info = refine_support(geo_init[i, j]...; abstol=0.0, reltol=0.0)
                D_exact[i, j] = info.curve_length
                results_exact[i, j] = info
            end
        end
    end
    t_exact = minimum(t_exact_rec)
    ...
    for r in 1:nrepeat
        t_tol_rec[r] = @elapsed begin
            @inbounds for i in 1:m, j in 1:n
                path, info = refine_support(geo_init[i, j]...; abstol=abstol, reltol=reltol)
                D_tol[i, j] = info.curve_length
                results_tol[i, j] = info
            end
        end
    end
    t_tol = minimum(t_tol_rec)
...
function compare_geodesic(treeA::PhyloTree, treeB::PhyloTree; 
    abstol=0.0, reltol=0.0,)

    initial, shared, inc = geodesic_initial(treeA, treeB) 
    ...
    display(gif(anim_geodesic, "tree_geodesic.gif"; fps=10))
    ...
    display(gif(anim_path, "tree_path.gif"; fps=10))
end